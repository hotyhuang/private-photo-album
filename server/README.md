...
```bash
aws-lambda-s3$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-lambda-s3$ sam local start-api
aws-lambda-s3$ curl http://localhost:3000/
```
...
```bash
aws-lambda-s3$ sam logs -n HelloWorldFunction --stack-name aws-lambda-s3 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name aws-lambda-s3
```
